...
func add_random_symbols():
	...
	for i in range(num_symbols):
		...
		symbol_nodes.append(symbol_node)
		if num_symbols - i <= 3:
			Utilities.symbol_nodes2.append(symbol_node)
			Utilities.rell_2.append(symbol_texture.get_path().substr(symbol_texture.get_path().rfind('/')
		 + 1, symbol_texture.get_path().length()))
		
	is_animation_playing = true
	Utilities.r2_is_animation_playing = true
...
func _on_button_button_down():
	if is_animation_playing == false:
		Utilities.r2_is_animation_playing = false
	if 	Utilities.r2_is_animation_playing == false:
		add_random_symbols()
